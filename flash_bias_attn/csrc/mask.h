...
struct Mask {
    ...
    const float * bias_table;      // per-head bias table (window_size entries), or nullptr
    const int bias_table_size;     // window_size for bias table

    __forceinline__ __device__ Mask(const int max_seqlen_k, const int max_seqlen_q,
                                    const int window_size_left, const int window_size_right,
                                    const float alibi_slope=0.f,
                                    const float *bias_table=nullptr,
                                    const int bias_table_size=0)
                    ...
                    for (int i = 0; i < size<0, 0>(tensor); ++i) {
                        ...
                        for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                            ...
                            for (int j = 0; j < size<1, 0>(tensor); ++j) {
                                ...
                                // Bias table lookup: bias_table[relative_pos + half_window]
